...
init_target() {
  ...
  local slug="$2"
  local work_repo="$3"
  "$ROOT/tools/collab/registry.py" init --agent-id codex --reviewer pa --no-participant-verification --work-repo "$work_repo" "$title" >/dev/null
  "$ROOT/tools/collab/registry.py" join-participants "$RUN_DATE-$slug" pa --agent-id opus >/dev/null
  "$ROOT/tools/collab/registry.py" join-participants "$RUN_DATE-$slug" pe --agent-id codex >/dev/null
...
"$ROOT/tools/collab/registry.py" execution "$AMEND_TARGET" pe completed "2026-05-23T18:00:00+02:00" \
  --assigned-role pe \
  --validation-result passed \
  --validation-scope scoped \
  --agent-id codex \
...
amend_state="$("$ROOT/tools/collab/registry.py" seal-state "$AMEND_TARGET" pa)"
amend_revision="$(read_json_field registryRevision <<<"$amend_state")"
"$ROOT/tools/collab/registry.py" seal-render "$AMEND_TARGET" pa --observed-revision "$amend_revision" --caller-role pa >/dev/null
...
assessment_state="$("$ROOT/tools/collab/registry.py" seal-state "$AMEND_TARGET" pa)"
assessment_revision="$(read_json_field registryRevision <<<"$assessment_state")"
"$ROOT/tools/collab/registry.py" seal-render "$AMEND_TARGET" pa --observed-revision "$assessment_revision" --caller-role pa --outcome success >/dev/null
...
"$ROOT/tools/collab/registry.py" execution "$DELETE_TARGET" pe completed "2026-05-23T18:00:00+02:00" \
  --assigned-role pe \
  --validation-result passed \
  --validation-scope scoped \
  --agent-id codex \
...
delete_state="$("$ROOT/tools/collab/registry.py" seal-state "$DELETE_TARGET" pa)"
delete_revision="$(read_json_field registryRevision <<<"$delete_state")"
"$ROOT/tools/collab/registry.py" seal-render "$DELETE_TARGET" pa --observed-revision "$delete_revision" --caller-role pa >/dev/null
