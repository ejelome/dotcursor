...
"$ROOT/tools/collab/registry.py" init --agent-id codex "Registry Schema Roundtrip" >/dev/null
...
"$ROOT/tools/collab/registry.py" set "$TARGET" description "Roundtrip mutation" --caller-role mod >/dev/null
...
set +e
output="$("$ROOT/tools/collab/registry.py" validate 2>&1)"
status=$?
set -e
