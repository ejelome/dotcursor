...
"$ROOT/tools/collab/registry.py" set "$PROSE_TARGET" active-phase Completion --force --caller-role mod >/dev/null
start_assessment "$PROSE_TARGET"
prose_revision="$(assessment_revision "$PROSE_TARGET")"
set +e
prose_output="$("$ROOT/tools/collab/registry.py" seal-render "$PROSE_TARGET" pa \
