...
"$ROOT/tools/collab/registry.py" init --agent-id codex --reviewer pa "Verification Restart Cycle" >/dev/null
...
run_pv() {
  ...
  state="$("$ROOT/tools/collab/registry.py" participant-verify-state "$TARGET" "$role")"
  revision="$(read_json_field registryRevision <<<"$state")"
  "$ROOT/tools/collab/registry.py" participant-verify-render "$TARGET" "$role" \
...
"$ROOT/tools/collab/registry.py" restart-verification "$TARGET" --caller-role pa >/dev/null
...
if "$ROOT/tools/collab/registry.py" restart-verification "$TARGET" --caller-role pe 2>/dev/null; then
...
if "$ROOT/tools/collab/registry.py" seal-render "$TARGET" pa --observed-revision "$seal_revision" --caller-role pa 2>/dev/null; then
