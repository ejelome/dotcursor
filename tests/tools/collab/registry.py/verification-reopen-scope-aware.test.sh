...
TW_PATH="tools/command-system/audit.sh"
PE_PATH="tools/command-system/audit-role-prose.sh"
...
reg() { "$ROOT/tools/collab/registry.py" "$@"; }
...
entry['handoff'] = {'roles': {
    'tw': {'writeScope': [tw_path], 'validationCommands': [['./tools/command-system/audit.sh']]},
    'pe': {'writeScope': [pe_path], 'validationCommands': [['./tools/command-system/audit.sh']]},
}}
