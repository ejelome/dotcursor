...
"$ROOT/tools/collab/registry.py" init --agent-id mod-agent "Participation Resume Output" >/dev/null
REGISTRY="$("$ROOT/tools/collab/registry.py" registry-path)"

join_output="$("$ROOT/tools/collab/registry.py" join-participants "$TARGET" pe --agent-id gpt)"
if [[ "$join_output" != *"RESUME: tools/collab/registry.py speak-state --resume $TARGET pe"* ]]; then
  printf 'FAIL: join advisory did not include exact resume command\n%s\n' "$join_output" >&2
  exit 1
fi
if [[ "$join_output" != *"TRANSCRIPT: tools/collab/registry.py transcript-view $TARGET Audit"* ]]; then
  printf 'FAIL: join advisory did not include active-phase transcript pointer\n%s\n' "$join_output" >&2
  exit 1
fi

"$ROOT/tools/collab/registry.py" join-participants "$TARGET" tw --agent-id sonnet >/dev/null
"$ROOT/tools/collab/registry.py" join-participants "$TARGET" pa --agent-id opus >/dev/null
"$ROOT/tools/collab/registry.py" set "$TARGET" turn-order "pe tw" --caller-role mod >/dev/null

state="$("$ROOT/tools/collab/registry.py" speak-state "$TARGET" pe --resume)"
...
assert state['nextTranscriptCommand'] == f'tools/collab/registry.py transcript-view {target} Audit', state
...
state="$("$ROOT/tools/collab/registry.py" speak-state "$TARGET" tw --resume)"
STATE="$state" python3 - "$TARGET" <<'PY'
import json
import os
import sys
...
"$ROOT/tools/collab/registry.py" set "$TARGET" active-phase Conclusion --force --caller-role mod >/dev/null
state="$("$ROOT/tools/collab/registry.py" speak-state "$TARGET" pe --resume)"
STATE="$state" python3 - "$TARGET" <<'PY'
import json
import os
...
state = json.loads(os.environ['STATE'])
assert state['nextCommand'] == f'/collab speak {target}', state
assert state['nextTranscriptCommand'] == f"tools/collab/registry.py transcript-view {target} 'Action Plan'", state
assert state['policyBlockers'] == [], state
...
state="$("$ROOT/tools/collab/registry.py" speak-state "$TARGET" pa --resume)"
STATE="$state" python3 - "$TARGET" <<'PY'
import json
import os
import sys
