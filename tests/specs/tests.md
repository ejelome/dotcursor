...
| `registry.py/role-file-readability.test.sh` (removed; consolidated) | `commands/collab/engine/registry.py` `validate_registry` rejects participants whose role source file is unreadable and reports `roles/ghost.json` | central-checker: `tests/commands/collab/modules.test.sh` | `structure` |
