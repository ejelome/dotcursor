...
- **`/narrative rewrite content <audit | align | gate> --role <key>`** — staged narrative rewrite workflow with a required per-phase role; `align` checks project-local rule files against their project overlay counterparts; `gate` reads `validationCommands` from `.revamps/<repo>-<date>.json`.
