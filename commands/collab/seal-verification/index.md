...
12. Collect every path listed in `execution.<role>.touchedPaths` across all execution entries. For each path, verify the path's final state — present or deleted — is committed in git, or is staged with no additional unstaged content. A committed deletion (a path removed by a commit) satisfies this gate: the deletion is recorded in git even though the file no longer exists on disk. A path deleted only in the working tree (not yet committed), or a touched path with unstaged content, means the implementation has not fully reached git and cannot be sealed. **ABORT**: implementation not in git; list each path that is unstaged and uncommitted.
...
- **Git-tracking gate:** `seal-render` enforces Step 12 by checking each execution `touchedPath` against git's commit history, index, and unstaged diff. Committed paths — including paths committed as deletions — and staged paths with no additional unstaged changes pass. A committed deletion passes because the path's final state (removed) is recorded in a commit, even though the file no longer exists on disk. Working-tree-only paths (including working-tree-only deletions) and touched paths with unstaged content fail with `SEAL-GIT-STATE: implementation not in git`. Sealing over uncommitted work would record paths in `verificationSeal.touchedPaths` whose content does not exist in git, so helper enforcement blocks the seal before writing.
- <a name="commit-reachability-gate"></a>**Commit-reachability gate:** On a `success` verdict, every recorded execution commit must be reachable from `HEAD` in the work repo. A commit that still exists but was orphaned by a rebase or re-commit is rejected with `SEAL-PROVENANCE` — the sealed deliverable must be in the current branch history. Record a non-success verdict to reopen, then re-execute against a committed, reachable commit before sealing. (`verificationSeal.stale` does not catch orphaning: the commit still exists; only its reachability changed.) For `workRepo` binding and reachability remediation items, see [`workRepo-remediation-index.md`](../../../core/collab/workRepo-remediation-index.md).
