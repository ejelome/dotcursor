...
12. Collect every path listed in `execution.<role>.touchedPaths` across all execution entries. For each path, verify the path has no unstaged work and is staged or committed in git — not working-tree-only. A path present only in the working tree, or a touched path with unstaged content after staging, means the implementation has not fully reached git and cannot be sealed. **ABORT**: implementation not in git; list each path that is unstaged and uncommitted.
...
- **Git-tracking gate:** `seal-render` enforces Step 12 by checking each execution `touchedPath` against git's index, staged diff, and unstaged diff. Committed paths and staged paths with no additional unstaged changes pass; working-tree-only paths and touched paths with unstaged content fail with `SEAL-GIT-STATE: implementation not in git`. Sealing over uncommitted work would record paths in `verificationSeal.touchedPaths` whose content does not exist in git, so helper enforcement blocks the seal before writing.
