...
- **Audit block schema — `charteredDeliverables`:** When the collab has explicit delivery commitments, the moderator's Audit contribution should include a `charteredDeliverables:` block listing each committed artifact as a bullet. The list is consumed at seal time by Invariant #17 to verify that every chartered item is covered by at least one cited committed path in the execution record. Format: each bullet is a short artifact description matching one planned deliverable (e.g., `- _invariants.md: add Invariant #16`). Omitting the block is permitted when no specific artifacts are chartered; Invariant #17 is a no-op for collabs with an empty or absent `charteredDeliverables` list.
