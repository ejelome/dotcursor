...
**Slash:** `/collab init`
**Signature:** `/collab init "<name>" [--reviewer <role>] [--participant-verification [--verification-cap <N>]] [--preview]`
...
3. Call `tools/collab/registry.py init --agent-id <agentId> [--reviewer <role>] [--participant-verification [--verification-cap <N>]] [--preview] <name>`. The helper owns the full transaction: strict flag parsing, unknown-flag rejection, title normalization, local-date resolution, slug derivation, sequence selection, moderator `agentId` capture, reviewer metadata, participant-verification metadata, transcript rendering, atomic registry/transcript replacement, and optional browser-open when `--preview` is supplied.
...
- **Registry side effect:** Successful init appends one collab entry to the resolved `registry.json`, includes the moderator role in `participants` and `turnOrder`, and sets `activeCollabId` to the new collab id. The transcript path is mirrored in `transcriptPath`. With `--participant-verification`, the entry starts with `verification = { rounds: 0, cap, subState: "participant", participantVerification: true, participants: {} }`.
<!-- abort: init-argument-validation -->
...
```cursor-arg
dispatch: (collab init "<name>" [--reviewer <role>] [--participant-verification [--verification-cap <N>]] [--preview])
...
param: name=--participant-verification; required=optional; placeholder=--participant-verification; class=type; rule=boolean flag
param: name=--verification-cap; required=optional; placeholder=<N>; class=type; rule=positive integer; requires=--participant-verification
