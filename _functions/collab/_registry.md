...
- **Per-collab entry fields:**
  ...
  | `verification.subState` | string | `"seal"` \| `"assessment"`. Active within `Completion.verification` for reviewer-backed collabs. Set to `"seal"` when verification begins; transitions to `"assessment"` after a successful seal. Re-enters `"assessment"` when the seal becomes stale or a cap-exit is recorded. |
...
- **Field ownership:**
  ...
  | `verification.subState` | `seal-render` (set to `seal` on verification start; transitions to `assessment` after successful seal or on stale/cap-exit) |
