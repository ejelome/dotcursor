...
Deterministic QA for shared role JSON sources projected to `~/.cursor/core/collab/_roles/`.

## Procedure

1. Load every `*.json` under the tracked source directory `core/collab/_roles/`.
...
6. Validate runtime mode copies `core/collab/_roles/` to `~/.cursor/core/collab/_roles/`.
...
_Generated from `core/collab/_roles/*.json`; do not edit this block by hand._

Tracked role source files under `core/collab/_roles/`:
